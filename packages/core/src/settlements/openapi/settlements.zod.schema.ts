import { makeApi, Zodios, type ZodiosOptions } from '@zodios/core';
import { z } from 'zod';

const executeSettlement_Body = z
  .object({ rail: z.enum(['fiat', 'crypto']) })
  .partial()
  .passthrough();
const SettlementStatus = z.enum(['pending', 'executed', 'blocked', 'failed']);
const Problem = z
  .object({
    type: z.string().url(),
    title: z.string(),
    status: z.number().int(),
    detail: z.string(),
    instance: z.string().url(),
    code: z.string(),
  })
  .partial()
  .passthrough();
const SettlementId = z.string();
const SettlementRail = z.enum(['fiat', 'crypto']);
const Settlement = z
  .object({
    settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
    rideId: z.string(),
    amount: z.number(),
    currency: z.string(),
    status: z.enum(['pending', 'executed', 'blocked', 'failed']),
    platformFeeRate: z.number(),
    platformFeeAmount: z.number().optional(),
    rail: z.enum(['fiat', 'crypto']),
    cryptoAllowed: z.boolean().optional(),
    attestationUnlockRequired: z.boolean().optional(),
    slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
    bankFloatBaselineHours: z.number().optional(),
    executedAt: z.string().datetime({ offset: true }).optional(),
    createdAt: z.string().datetime({ offset: true }),
    updatedAt: z.string().datetime({ offset: true }),
  })
  .passthrough();
const SettlementListData = z
  .object({
    items: z.array(
      z
        .object({
          settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
          rideId: z.string(),
          amount: z.number(),
          currency: z.string(),
          status: z.enum(['pending', 'executed', 'blocked', 'failed']),
          platformFeeRate: z.number(),
          platformFeeAmount: z.number().optional(),
          rail: z.enum(['fiat', 'crypto']),
          cryptoAllowed: z.boolean().optional(),
          attestationUnlockRequired: z.boolean().optional(),
          slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
          bankFloatBaselineHours: z.number().optional(),
          executedAt: z.string().datetime({ offset: true }).optional(),
          createdAt: z.string().datetime({ offset: true }),
          updatedAt: z.string().datetime({ offset: true }),
        })
        .passthrough()
    ),
    nextCursor: z.string().optional(),
  })
  .passthrough();
const ResponseMeta = z
  .object({
    requestId: z.string().uuid(),
    correlationId: z.string(),
    generatedAt: z.string().datetime({ offset: true }),
  })
  .partial()
  .passthrough();
const SettlementListResponse = z
  .object({
    data: z
      .object({
        items: z.array(
          z
            .object({
              settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
              rideId: z.string(),
              amount: z.number(),
              currency: z.string(),
              status: z.enum(['pending', 'executed', 'blocked', 'failed']),
              platformFeeRate: z.number(),
              platformFeeAmount: z.number().optional(),
              rail: z.enum(['fiat', 'crypto']),
              cryptoAllowed: z.boolean().optional(),
              attestationUnlockRequired: z.boolean().optional(),
              slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
              bankFloatBaselineHours: z.number().optional(),
              executedAt: z.string().datetime({ offset: true }).optional(),
              createdAt: z.string().datetime({ offset: true }),
              updatedAt: z.string().datetime({ offset: true }),
            })
            .passthrough()
        ),
        nextCursor: z.string().optional(),
      })
      .passthrough(),
    meta: z
      .object({
        requestId: z.string().uuid(),
        correlationId: z.string(),
        generatedAt: z.string().datetime({ offset: true }),
      })
      .partial()
      .passthrough()
      .optional(),
  })
  .passthrough();
const SettlementResponse = z
  .object({
    data: z
      .object({
        settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
        rideId: z.string(),
        amount: z.number(),
        currency: z.string(),
        status: z.enum(['pending', 'executed', 'blocked', 'failed']),
        platformFeeRate: z.number(),
        platformFeeAmount: z.number().optional(),
        rail: z.enum(['fiat', 'crypto']),
        cryptoAllowed: z.boolean().optional(),
        attestationUnlockRequired: z.boolean().optional(),
        slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
        bankFloatBaselineHours: z.number().optional(),
        executedAt: z.string().datetime({ offset: true }).optional(),
        createdAt: z.string().datetime({ offset: true }),
        updatedAt: z.string().datetime({ offset: true }),
      })
      .passthrough(),
    meta: z
      .object({
        requestId: z.string().uuid(),
        correlationId: z.string(),
        generatedAt: z.string().datetime({ offset: true }),
      })
      .partial()
      .passthrough()
      .optional(),
  })
  .passthrough();
const SettlementExecuteRequest = z
  .object({ rail: z.enum(['fiat', 'crypto']) })
  .partial()
  .passthrough();

export const schemas: any = {
  executeSettlement_Body,
  SettlementStatus,
  Problem,
  SettlementId,
  SettlementRail,
  Settlement,
  SettlementListData,
  ResponseMeta,
  SettlementListResponse,
  SettlementResponse,
  SettlementExecuteRequest,
};

const endpoints = makeApi([
  {
    method: 'get',
    path: '/v1/settlements',
    alias: 'listSettlements',
    requestFormat: 'json',
    parameters: [
      {
        name: 'cursor',
        type: 'Query',
        schema: z.string().optional(),
      },
      {
        name: 'status',
        type: 'Query',
        schema: z.enum(['pending', 'executed', 'blocked', 'failed']).optional(),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            items: z.array(
              z
                .object({
                  settlementId: z
                    .string()
                    .regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
                  rideId: z.string(),
                  amount: z.number(),
                  currency: z.string(),
                  status: z.enum(['pending', 'executed', 'blocked', 'failed']),
                  platformFeeRate: z.number(),
                  platformFeeAmount: z.number().optional(),
                  rail: z.enum(['fiat', 'crypto']),
                  cryptoAllowed: z.boolean().optional(),
                  attestationUnlockRequired: z.boolean().optional(),
                  slaDeadlineAt: z
                    .string()
                    .datetime({ offset: true })
                    .optional(),
                  bankFloatBaselineHours: z.number().optional(),
                  executedAt: z.string().datetime({ offset: true }).optional(),
                  createdAt: z.string().datetime({ offset: true }),
                  updatedAt: z.string().datetime({ offset: true }),
                })
                .passthrough()
            ),
            nextCursor: z.string().optional(),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'get',
    path: '/v1/settlements/:settlementId',
    alias: 'getSettlement',
    requestFormat: 'json',
    parameters: [
      {
        name: 'settlementId',
        type: 'Path',
        schema: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
            rideId: z.string(),
            amount: z.number(),
            currency: z.string(),
            status: z.enum(['pending', 'executed', 'blocked', 'failed']),
            platformFeeRate: z.number(),
            platformFeeAmount: z.number().optional(),
            rail: z.enum(['fiat', 'crypto']),
            cryptoAllowed: z.boolean().optional(),
            attestationUnlockRequired: z.boolean().optional(),
            slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
            bankFloatBaselineHours: z.number().optional(),
            executedAt: z.string().datetime({ offset: true }).optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 404,
        description: `Resource not found`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'post',
    path: '/v1/settlements/:settlementId/execute',
    alias: 'executeSettlement',
    requestFormat: 'json',
    parameters: [
      {
        name: 'body',
        type: 'Body',
        schema: executeSettlement_Body.optional(),
      },
      {
        name: 'settlementId',
        type: 'Path',
        schema: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
      },
      {
        name: 'Idempotency-Key',
        type: 'Header',
        schema: z.string().min(1).max(128),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
            rideId: z.string(),
            amount: z.number(),
            currency: z.string(),
            status: z.enum(['pending', 'executed', 'blocked', 'failed']),
            platformFeeRate: z.number(),
            platformFeeAmount: z.number().optional(),
            rail: z.enum(['fiat', 'crypto']),
            cryptoAllowed: z.boolean().optional(),
            attestationUnlockRequired: z.boolean().optional(),
            slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
            bankFloatBaselineHours: z.number().optional(),
            executedAt: z.string().datetime({ offset: true }).optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 404,
        description: `Resource not found`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 409,
        description: `Idempotency key reuse with different body, or state conflict`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
  {
    method: 'post',
    path: '/v1/settlements/:settlementId/retry',
    alias: 'retrySettlement',
    requestFormat: 'json',
    parameters: [
      {
        name: 'body',
        type: 'Body',
        schema: executeSettlement_Body.optional(),
      },
      {
        name: 'settlementId',
        type: 'Path',
        schema: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
      },
      {
        name: 'Idempotency-Key',
        type: 'Header',
        schema: z.string().min(1).max(128),
      },
    ],
    response: z
      .object({
        data: z
          .object({
            settlementId: z.string().regex(/^stl_[0-9A-HJKMNP-TV-Z]{26}$/),
            rideId: z.string(),
            amount: z.number(),
            currency: z.string(),
            status: z.enum(['pending', 'executed', 'blocked', 'failed']),
            platformFeeRate: z.number(),
            platformFeeAmount: z.number().optional(),
            rail: z.enum(['fiat', 'crypto']),
            cryptoAllowed: z.boolean().optional(),
            attestationUnlockRequired: z.boolean().optional(),
            slaDeadlineAt: z.string().datetime({ offset: true }).optional(),
            bankFloatBaselineHours: z.number().optional(),
            executedAt: z.string().datetime({ offset: true }).optional(),
            createdAt: z.string().datetime({ offset: true }),
            updatedAt: z.string().datetime({ offset: true }),
          })
          .passthrough(),
        meta: z
          .object({
            requestId: z.string().uuid(),
            correlationId: z.string(),
            generatedAt: z.string().datetime({ offset: true }),
          })
          .partial()
          .passthrough()
          .optional(),
      })
      .passthrough(),
    errors: [
      {
        status: 401,
        description: `Missing or invalid API key`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
      {
        status: 404,
        description: `Resource not found`,
        schema: z
          .object({
            type: z.string().url(),
            title: z.string(),
            status: z.number().int(),
            detail: z.string(),
            instance: z.string().url(),
            code: z.string(),
          })
          .partial()
          .passthrough(),
      },
    ],
  },
]);

export const api: any = new Zodios(
  'https://api.ddd-codegen-starter.local/v1',
  endpoints
);

export function createApiClient(baseUrl: string, options?: ZodiosOptions): any {
  return new Zodios(baseUrl, endpoints, options);
}
